...
pub async fn nats_server(
    ...
    let requests = client.subscribe(format!("ipc.{}", currencyid)).await?;

    tokio::spawn(async move {
        listen(cs_tx, client, requests, currencyid).await?;
...
pub async fn listen(
    ...
    while let Some(request) = requests.next().await {
        ...
        if let Some(reply) = request.reply {
            let mut payload: Payload = serde_json::from_slice::<Payload>(request.payload.as_ref())?;
            ...
            match &*payload.command {
                ...
                "setstaking" => {
                    ...
                    if let Ok(_res) = os_rx.await {
                        client
                            .publish(reply, json!({ "result": "success" }).to_string().into())
                            .await?
                    } else {
                        client
                            .publish(reply, json!({ "result": "failed" }).to_string().into())
                            .await?
                    }
                }
                ...
                "newpendingsubscriber" => {
                    let (os_tx, os_rx) = oneshot::channel::<Result<Subscriber, CoinStakerError>>();
                    ...
                    match os_rx.await? {
                        Ok(sub) => {
                            ...
                            client
                                .publish(reply, json!({ "result": serde_str }).to_string().into())
                                .await?
                        }
                        Err(e) => {
                            client
                ...
                "getsubscriptions" => {
                    ...
                    client
                        .publish(reply, json!({ "result": &subscribers }).to_string().into())
                        .await?
                }
                ...
                "getidentity" => {
                    ...
                    if let Some(identity) = os_rx.await? {
                        ...
                        client
                            .publish(reply, json!({"result": "not found"}).to_string().into())
                            .await?
                    }
                }
                // arguments: identity
